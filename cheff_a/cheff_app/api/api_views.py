...
class GetPtAPIView(ListAPIView):
    __doc__ = f'''{'GET'} 
    `[METODO GET]`
    Esta vista de API nos devuelve una lista de todos los productos terminados presentes 
    en la base de datos.
    '''
...
class PostPtAPIView(CreateAPIView):
    __doc__ = f'''{''}
    `[METODO POST]`
    Esta vista de API nos permite hacer un insert en la tabla Pt.
    '''
    queryset = ProductoTerminado.objects.all()
    serializer_class = ProductoTerminadoSerializer
    permission_classes = [IsAuthenticated, IsAdminUser]
